...
def prev_url(target, calendar, period):
    ...
    return mark_safe('<a href="%s%s"><span class="fas fa-arrow-left"></span></a>' % (
...
def next_url(target, calendar, period):
    ...
    return mark_safe('<a href="%s%s"><span class="fas fa-arrow-right"></span></a>' % (
        reverse(target, kwargs={'calendar_slug': slug}),
...
def hash_occurrence(occ):
    return 'accurance%s_%s' % (occ.start.strftime('%Y%m%d%H%M%S'), occ.event.id)
